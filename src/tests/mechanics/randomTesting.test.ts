...
describe(   "Random Games", () => {
    ...
    it("100 rounds", async () => {
        // throw new Error("This test is meant to be run manually with a debugger attached, it will fail if run in CI. Set TEST_RELOAD to true to enable game reload from logs at the end of the test.");
        ...
        let currentRound = 0;
        let seen = false;
        while(game.turnHandler.round < 100 && game.turnHandler.numberOfRoundSinceBeginning < 500) {
            uniqueIdCounter++;
            ...
            // console.log(game.entitiesInCombat.map(e => e.id));
            // console.log("stack size", game.stack.elements.length);
            // console.log("attack", game.getAttack(game.currentPlayer));
            // console.log("cod: ", game.monsters[0]?.isDead);
            // console.log("active Rooms: ", game.rooms?.activeRooms.map(r => r ? r.name : "empty").join(", "));
            ...
            // console.log("Players attackable: ", game.players.filter(p => p.attackable).map(p => p.id).join(", "));
            // printVerbose("attack: ", bot.me.attackThisTurn, " free attacks remaining: ", bot.me.hasFreeAttackRemaining, " has attack requirement: ", bot.me.hasAttackRequirement);
            // console.log("entities in combat", game.entitiesInCombat.map(e => e.id), "selected target");
            // console.log(`dice modifier: ${game.currentPlayer.diceModifier}, attack dice modifier: ${game.currentPlayer.attackDiceModifier}`);
            ...
            // console.log("Monsters: ", game.monsters.map(m => `${m.card.name}(${m.currentHealthPoints}HP)${m.isEngagedInCombat ? " engaged" : ""}${m.isDead ? " dead" : ""}`).join(", "));
    ...
    function displayPlentyOfInfo(bot: Bot) {
        ...
        printVerbose("can end turn:", game.canEndTurn(bot.me, false));
        printVerbose("Can declare attack: ", game.canDeclareAttack(bot.me, false));
