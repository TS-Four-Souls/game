...
//     it("100 rounds", async () => {
//         // throw new GameError("This test is currently disabled as it can be very flaky and doesn't provide consistent value. It can be re-enabled for specific seeds that are known to cause issues, or after improving the bot's decision making to reduce the chances of it getting stuck in loops or bad states.");
...
//                 throw new GameError("Unique ID counter exceeded 100000, there is likely an infinite loop in the test.");
...
//             //     throw new GameError("The Harbingers is present in the encounter slots, which should not happen as it is not included in the monster pool. This may indicate a bug in monster spawning or state updates between turns.");
...
//         //         {
//         //             throw new GameError(`Card in treasure DISCARD is not a TreasureCard: ${card.name}, ${card.globalId}`);
//         //         }
//         //      for(const card of game.decks.treasure.cards)
//         //         if((card.type !== "treasure"))
//         //         {
//         //             throw new GameError(`Card in treasure deck is not a TreasureCard: ${card.name}, ${card.globalId}`);
//         //         }
//         //     if(dupli.length > 0)
//         //         throw new GameError(`Duplicate monsters found in encounter slots: ${game.encounters.visible.map(m => m ? m.slug : "empty").join(", ")}, dupliactes: ${dupli.join(", ")}`);
...
//         //     if(dupl.length > 0)
//         //         throw new GameError(`Duplicate cards found in the loot deck. ${game.decks.loot._set.cards.find(c => c.id === dupl[0])?.name}, ${game.decks.loot._discard.includes(dupl[0])}, ${game.players.flatMap(p=>[...p.hand.cards.map(c => c.id), ...p.inPlay.filter(c=> c instanceof LootCard).map(c => c.id)]).includes(dupl[0])}, ${game.decks.loot._order.includes(dupl[0])}`);
//         //    if(bot1.me.inPlay.some(c => c.owner !== bot1.me) || bot2.me.inPlay.some(c => c.owner !== bot2.me))
//         //         throw new GameError(`Player ${bot1.me.id} or ${bot2.me.id} has an item (${bot1.me.inPlay.find(c => c.owner !== bot1.me)?.name || bot2.me.inPlay.find(c => c.owner !== bot2.me)?.name}) in play that is not owned by them. This should not happen and may indicate a bug in state updates between turns.`);
//         //     if(game.shop.itemsInShop.some(i => i === undefined))
//         //         throw new GameError("Shop deck is empty, which should not happen. Check for potential bugs in card obtaining or shop refill." + game.shop._deck.length + ", discard: " + game.shop._deck.discard.length);
//             //     console.log("Current monsters in slots:", game.monsters.map(m => m ? m.card.name : null));
...
//             //     console.log("Current monsters in slots:", game.monsters.map(m => m ? m.card.name : null));
//             //     console.log("Current monsters in slots:", game.encounters.cardsOnTop.map(m => m ? m.name : null));
//             //     throw new GameError("A monster is in the encounter monsters list but not present in the encounter slots, which should not happen. This may indicate a bug in monster spawning or state updates between turns.");
//             // }
...
//             //     throw new GameError(`Player ${bot.me.id} has less than 2 items in play, which should not happen as the starting item is eternal. This may indicate a bug in item removal or state updates between turns.`);
//             // if(game.entityHandler.getAttack(game.currentPlayer)=== 0)
//             //     throw new GameError("Player has 0 attack, but it's still their turn. This should not happen.");
...
//             //     throw new GameError("Player has 5 or more dice modifier, which should not be possible. Check for potential bugs in stat modification or stacking.");
//             if(action === null && bot.me !== game.currentPlayer)
//                 continue;
//             if(action === null) {
//                 displayPlentyOfInfo(bot);
//                 throw new GameError(`Seed: ${game.seed}, Player ${bot.me.id} has no playable actions at round ${game.turnHandler.round}. This should not happen.`);
//             }
//             // if(!seen && game.monsters.some(m => m.card.slug === "b2-cod_worm"))
//             //     seen = true;
//             // if((seen && !game.monsters.some(m => m.card.slug === "b2-cod_worm")))
//             //     throw new GameError("Nerve Ending was seen but is no longer present, it may have been killed or removed from combat. This should be possible but is worth investigating if it happens consistently with the same seed.");
...
//                         throw new GameError("Expected PurchaseAction");
...
//                         throw new GameError("Expected UseItemAction");
...
//                         throw new GameError(`Bot tried to use item ${action.item.name} which is not in their in-play area. This should not happen and may indicate a bug in action feasibility checking or state updates between action selection and execution.`);
//                     try {
//                         if(bot.game.actions.canActivate(action.item, bot.me) !== true)
//                             throw new GameError(`Item ${action.item.name} cannot be activated by player ${bot.me.id}`);
//                         const result = TargetBuilder.buildRandomValidTargets(game, bot.me, action.item, "inPlay");
//                         if(typeof result === "string")                         
//                             throw new GameError(`Failed to build targets for item ${action.item.name}: ${result} ${game.players.map(p => p.inPlay.length)}`);
...
//                         throw new GameError("Expected PlayLootAction");
...
//                         throw new GameError(`Failed to build targets for loot card ${card.name}: ${res}`);
//                     if (typeof res !== "string") {
...
//                         throw new GameError("Expected DeclareAttackOnEntityAction");
...
//                     //     throw new GameError("Stack contains Tech X steal effect, which can cause infinite loops if the bot keeps trying to resolve it while there are no coins to steal. This should be investigated if it happens consistently with the same seed.");
//                     if(game.stack.size > 10000) {
//                             game.stack.elements.forEach(e => {
//                                 console.log(`  Stack element ${e.debugLogs}`);
//                             });
//                         throw new GameError(`Stack size exceeded 1000 "${game.seed}"`);
//                     }
//                     break;
...
//                     break;
//                 default:
//                     throw new GameError(`Unhandled action type: ${action.type}`);
//             }
//             try {
