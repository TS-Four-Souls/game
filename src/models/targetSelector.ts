...
export function targetSelectorParser(s: string, game: Game): TargetsSelector[] {
    ...
    if (s.includes(" if you do, "))
        return [{ description: "If you do,", selector: IfYouDoTargetSelector(s, game), count: 1, asMany: false }];
    if (s.startsWith("choose one-"))
        return [{ description: "Choose one-", selector: chooseOneTargetSelector(s, game), count: 1, asMany: false }];
    if (s === "give another non-eternal item you control to another player:" ||
        ...
        coinStolen !== null) {
        return [{ description: "Choose another player", selector: anotherPlayerSelector(undefined, game), count: 1, asMany: false }];
    }
    if (s === "cancel the ↷ or $ ability of an item.")
        return [{ description: "Select a loot card on the stack.", selector: stackElementSelector((element) => element instanceof EffectOnStack, game), count: 1, asMany: false }];
    if (s === "cancel the ↷ or $ ability of an item or a loot being played.")
        return [{ description: "Select a loot card on the stack.", selector: stackElementSelector((element) => element instanceof LootCardEffect || element instanceof EffectOnStack, game), count: 1, asMany: false }];
    if (s.startsWith("choose a player.") ||
        s === "kill a player.") {
        return [{ description: "Choose a player", selector: playerSelector(undefined, game), count: 1, asMany: false }];
    }
    if (s === "[paid effect] remove 3 counters from this:\nkill a player or monster.")
        return [{ description: "Choose a player or monster", selector: activeEntitySelector(undefined, game), count: 1, asMany: false }];
    if (s === "choose the player with the most souls or tied for the most. that player destroys a soul they control.") {
        return [{ description: "Choose a player with the most souls or tied for the most.", selector: playerSelector((p) => p.souls.length === Math.max(...game.players.map(p => p.souls.length)), game), count: 1, asMany: false }];
    }
    if (s === "choose a dice roll. its controller rerolls it." ||
        ...
        return [{ description: "Choose a dice roll", selector: rollSelector(undefined, game), count: 1, asMany: false }];
    }
    if (s === "add or subtract 1 from a roll.") {
        return [{ description: "Choose a dice roll", selector: rollSelector(undefined, game), count: 1, asMany: false },
        { description: "Choose to add or subtract 1", selector: (issuer: Player) => [1, -1], count: 1, asMany: false }
        ];
    }
    if (s === "choose a monster. the active player must attack that monster this turn if able.") {
        return [{ description: "Choose a monster", selector: (issuer: Player) => game.monsters, count: 1, asMany: false }];
    }
    if (s === "choose a non-eternal passive item.")
        return [{ description: "Choose a non-eternal passive item", selector: inplayItemSelector((player: Player, card: ItemCard) => card.eternal === false && card.subtype === "passive", game), count: 1, asMany: false }];
    if (s === "choose a non-eternal item. this becomes a copy of that item.\n(this change is indefinite.)")
        return [{ description: "Choose a non-eternal item", selector: inplayItemSelector((player: Player, card: ItemCard) => card.eternal === false, game), count: 1, asMany: false }];
    ...
    if (s === "destroy an item you control.") {
        return [{ description: "Destroy an item you control", selector: inplayItemSelector((player: Player, card: ItemCard) => card.eternal === false && player == game.getOwner(card), game), count: 1, asMany: false }];
    }
    if (s === "destroy a curse.")
        return [{ description: "Select a curse.", selector: inplayCurseSelector((player, card) => true, game), count: 1, asMany: false }];
    if (s === "recharge an item.") {
        return [{ description: "Select a rechargeable item", selector: inplayUnchargedItemSelector(game), count: 1, asMany: false }];
    }
    if (s === "steal a non-eternal item from a player or from the shop.") {
        return [{ description: "Select a non-eternal item from a player or from the shop", selector: visibleItemSelector((card: ItemCard) => card.eternal === false, game), count: 1, asMany: false }];
    }
    if (s === "destroy a soul you control.") {
        return [{ description: "Destroy a soul you control", selector: (issuer: Player) => issuer.souls, count: 1, asMany: false }];
    }
    if (s === "look at the top 5 cards of a deck. put them back in any order."
        || s === "put the top card of any discard on top of its deck."
    )
        return [{ description: "Select a deck", selector: deckSelector(undefined, game), count: 1, asMany: false }];
    ...
    //         }}];
    return [{ description: "", selector: (issuer: Player) => [], count: 0, asMany: false }];
}
