...
export abstract class Slots<T extends Card> {
    ...
    draw(position: number) : void {
        ...
        if(card === undefined)
            throw new GameError(`Cannot draw card from deck for slot ${position}.`, toSerializedTranslation("error2.behaviorError", {error: `Cannot draw card from deck for slot ${position}.`}));
