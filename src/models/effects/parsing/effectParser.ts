 ...
 *  Then, each time the trigger condition is met, the rest of the effect will be executed.
 */
function parseTriggeredEffect(s: string, game: Game, nr: NumberRobustString, selectionOnResolve: boolean, youMayEffectHanging: boolean[]): ParsedEffect | null {
if (s.startsWith("when you die, ")) {
    ...
    if(s.startsWith("each time you miss an attack roll, ")){
        const restParsed = effectParser(s.substring(s.indexOf(",") + 1).trim(), game, true);
        return noTargetEffect( passive.onYourEventEffect("on:attack:roll:failed", [restParsed.effectFunction], game, s));
    }
    ...
    if (s.startsWith("each time a monster or player dies, "))
        return noTargetEffect(passive.onAnyEventEffect("on:death:before-penalty", [effectParser(s.substring(s.indexOf(",") + 1).trim(), game, true).effectFunction], game, s, 
 ...
 */
export function parseRollEffect(s: string, nr: NumberRobustString, game: Game, issuerIsCurrentPlayer: boolean = false): ParsedEffect {
    switch (nr.masked){
...
function parseStandardEffect(s: string, game: Game, nr: NumberRobustString, selectionOnResolve: boolean, youMayEffectHanging: boolean[]): ParsedEffect | null {
    ...
    switch (nr.normalizedMasked) {
        ...
        case "remove x counters from this":
            return noTargetEffect(active.removeCountersEffect(game, nr.nextNumber()));
        case "remove a counter from this":
            return noTargetEffect(active.removeCountersEffect(game, 1));
        case "players who control the fewest souls or tied for fewest may purchase a shop item for x¢ on their turn":
            return noTargetEffect(room.playersWithFewestSoulsShopItemPriceReductionEffect(game, nr.nextNumber()));
        case "gain x¢":
            return noTargetEffect(active.gainCoinsEffect(game, nr.nextNumber()));
        ...
        case "steal x¢ from them":
            return noTargetEffect(active.stealCoinsEffect(game, nr.nextNumber()));
        case "gain x treasure":
        case "gain x treasures":
            return noTargetEffect(active.gainTreasuresEffect(game, nr.nextNumber()));
        case "each monster heals x [hp]":
            return noTargetEffect(active.healEachMonsterEffect(game, nr.nextNumber()));
        case "heal x [hp]":
        case "this heals x [hp]":
            return noTargetEffect(active.healEffect(game, nr.nextNumber()));
        case "lose x¢":
            return noTargetEffect(active.loseCoinsEffect(game, nr.nextNumber()));
        case "loot x":
            return noTargetEffect(active.lootCardsEffect(game, nr.nextNumber()));
        case "remove x counter from this":
        case "remove x counters from this":
            return noTargetEffect(active.removeCountersFromThisEffect(game, nr.nextNumber()));
        case "pay x [hp]":
            return noTargetEffect(active.payHealthEffect(game, nr.nextNumber()));
        case "pay x¢":
        case "pay x¢:":
            return noTargetEffect(active.payCoinsEffect(game, nr.nextNumber()));
        ...
        case "each player gains x¢":
            return noTargetEffect(active.eachPlayerGainsCoinsEffect(game, nr.nextNumber()));
        ...
        case "players can be attacked and have x+ [dc]":
            return noTargetEffect(room.otherPlayersAreAttackableEffect(game, nr.nextNumber()));
        case "this heals x [hp]":
            return noTargetEffect(monster.thisHealsEffect(game, nr.nextNumber()));
        case "each player loses x¢":
            return noTargetEffect(active.eachPlayerLosesCoinsEffect(game, nr.nextNumber()));
        case "each player takes x damage":
        case "deal x damage to each player":
            return noTargetEffect(active.dealDamageToEachPlayerEffect(game, nr.nextNumber()));
        case "each monster takes x damage":
        case "deal x damage to each monster":
            return noTargetEffect(active.dealDamageToEachMonsterEffect(game, nr.nextNumber()));
        case "shop items you purchase cost x¢ less":
            return noTargetEffect(passive.shopItemsCostLessEffect(nr.nextNumber(), game));
        case "you take x damage":
            return noTargetEffect(active.takeDamageEffect(game, nr.nextNumber(), true));
        case "take x damage":
        case "this takes x damage":
            return noTargetEffect(active.takeDamageEffect(game, nr.nextNumber()));
        case "take x damage and gain x¢":
            return noTargetEffect(active.takeDamageGainCoinsEffect(s, nr.nextNumber(), nr.nextNumber(), game));
        case "deal x damage to a monster or player":
        case "deal x damage to a player":
        case "deal x damage to a monster":
            return { effectFunction: active.dealDamageToTargetEffect(game, nr.nextNumber(), selectionOnResolve, decideEntitySelector(s, game)), targetSelectors: decideEntitySelector(s, game) };
        case "deal x damage to them":
            return { effectFunction: active.dealDamageToTargetEffect(game, nr.nextNumber(), false, decideEntitySelector(s, game)), targetSelectors: decideEntitySelector(s, game) };
        ...
        case "when this is flipped to this side, loot x":
            return noTargetEffect(passive.lootAfterFlippingEffect(game, nr.nextNumber()));
        ...
        case "each time a player dies, before paying penalties, loot x":
            return noTargetEffect(passive.lootOnPlayerDeathEffect(nr.nextNumber(), game));
        case "if you would gain any number of ¢, gain that much x¢ instead":
            return noTargetEffect(passive.gainPlusCoinsEffect(nr.nextNumber(), game));
        case "this item starts with x counters on it":
            return noTargetEffect(passive.startWithNCountersEffect(nr.nextNumber(), game));
        ...
        case "you have x [atk] for your first attack roll each turn":
            return noTargetEffect(passive.firstAttackRollStatModifierEffect(nr.nextNumber(), 0, 0, game));
        ...
        case "each player rolls-\nx-x: they skip their next turn":
            return noTargetEffect(active.eachPlayerRollsSkipNextTurnEffect(game, nr.nextNumber(), nr.nextNumber()));
        ...
        case "each time another player gains ¢, they must give you x¢":
            return noTargetEffect(passive.stealCoinOnGainEffect(nr.nextNumber(), game));
        ...
        case "each other player takes x damage":
            return noTargetEffect(active.dealDamageToEachOtherPlayerEffect(game, nr.nextNumber()));
        ...
        case "combat damage you take is doubled on attack rolls of x":
            return noTargetEffect(passive.combatDamageModifierOnAttackRollEffect(game, [nr.nextNumber()], "double", "taken"));
        case "combat damage you deal is doubled on attack rolls of x":
            return noTargetEffect(passive.combatDamageModifierOnAttackRollEffect(game, [nr.nextNumber()], "double", "dealt"));
        ...
        case "each time the active player deals damage to this, they roll-\nx-x: they take x damage.\nx-x: each player takes x damage.\nx-x: this takes x damage":
            return noTargetEffect(monster.OnDamageByActivePlayerRollDealDamageEffect(game, nr.numbers));
        ...
        case "this deals x damage to a player chosen at random":
            return noTargetEffect(active.dealDamageToRandomPlayerEffect(game, nr.nextNumber(), "any"));
        case "deal x damage to a non-active player chosen at random":
            return noTargetEffect(active.dealDamageToRandomPlayerEffect(game, nr.nextNumber(), "non-active"));
        ...
        case "put this in the monster deck x cards from the top":
            return noTargetEffect(monster.putInMonsterDeckNFromTopEffect(game, nr.nextNumber()));
        case "choose a player. they take x damage. put this in the monster deck x cards from the top":
            return noTargetEffect(active.combineEffectFunctions([
                ...
                active.dealDamageToTargetEffect(game, nr.nextNumber()),
        ...
        case "each time this deals combat damage to the attacking player, it deals x damage to each other player":
            return noTargetEffect(passive.onAnyEventEffect("on:damage:taken", [active.dealDamageToEachPlayerEffect(game, nr.nextNumber(), false)], game, s, 
        ...
        case "when this dies, it deals x damage to the player who killed it":
            return noTargetEffect(monster.dealDamageToKillerOnDeathEffect(game, nr.nextNumber()));
        case "put it in the monster deck x cards from the top":
            return noTargetEffect(monster.putInMonsterDeckNFromTopEffect(game, nr.nextNumber()));
        case "when this dies on an attack roll of x, double its rewards":
            return noTargetEffect(monster.doubleRewardsOnDeathRollEffect(game, [nr.nextNumber()]));
        case "it deals x damage to each player":
            return noTargetEffect(active.dealDamageToEachPlayerEffect(game, nr.nextNumber()));
        case "deal x damage to each monster and player":
            return noTargetEffect(active.dealDamageToEachMonsterAndPlayerEffect(game, nr.nextNumber()));
        ...
        case "monsters have x [dc]":
            return noTargetEffect(monster.monstersGainDCEffect(game, nr.nextNumber(), true));
        case "monsters have x [hp]":
            return noTargetEffect(monster.monstersGainHPEffect(game, nr.nextNumber()));
        case "it heals x [hp]":
            return noTargetEffect(active.healEffect(game, nr.nextNumber()));
        case "look at the top x cards of the monster deck and put them back in any order":
            return noTargetEffect(active.lookAndReorderTopCardsEffect(game, nr.nextNumber(), "monster", "currentPlayer"));
        ...
        case "deal x damage to the player to the active player's left":
            return noTargetEffect(monster.dealDamageToPlayerToTheEffect(game, nr.nextNumber(), "left"));
        case "the player to your right takes x damage":
        case "deal x damage to the player to your right":
            return noTargetEffect(monster.dealDamageToPlayerToTheEffect(game, nr.nextNumber(), "right"));
        ...
        case "look at the top x cards of the loot deck. put them back in any order":
            return noTargetEffect(active.lookAndReorderTopCardsEffect(game, nr.nextNumber(), "loot"));
        case "when any player controls a soul, players who control the most souls or tied for the most must pay each other player x¢ to attack":
            return noTargetEffect(room.payOtherPlayersToAttackEffect(game, nr.nextNumber()));
        ...
        case "if this has x+ counters, flip it":
            return noTargetEffect(monster.flipIfXCountersEffect(game, nr.nextNumber()));
        case "if this would die, instead put a counter on this, the active player gains x treasure, cancels their attack and this heals to full [hp]":
            return noTargetEffect(monster.preventDeathGainTreasureCancelAttackAndHealEffect(game, nr.nextNumber()));
        ...
        case "the first time this would die each turn, prevent death. this heals x [hp] and gains x [dc] and -x [atk] till end of turn":
            return noTargetEffect(monster.preventDeathFirstTimeEachTurnHealAndStatModifierEffect(game , nr.nextNumber(), nr.nextNumber(), -nr.nextNumber()));
        ...
        case "shop items the active player purchases cost x¢ less":
            return noTargetEffect(room.cheaperShopItemsEffect(game, nr.nextNumber()));
        case "each time a player declares an attack, before choosing what to attack, they may look at the top x cards of the monster deck and put them back in any order":
            return noTargetEffect(room.lookAtTopNOnAttackEffect(game, nr.nextNumber()));
        case "each time a player declares an attack, the active player rolls-\nx-x: this deals x damage to that player":
            return noTargetEffect(monster.dealDamageOnAttackDeclarationEffect(game, nr.nextNumber(),nr.nextNumber(), nr.nextNumber()));
        case "each time a player dies, each other player gains x¢":
            return noTargetEffect(room.gainCoinsOnPlayerDeathEffect(game, nr.nextNumber()));
        case "each time a player dies, each other player loots x":
            return noTargetEffect(room.lootOnPlayerDeathEffect(game, nr.nextNumber()));
        case "at the end of the turn, the active player loses x¢":
            return noTargetEffect(room.loseCoinsAtEndOfTurnEffect(game, nr.nextNumber()));
        ...
        case "note each goal as players complete them. this room can't be put into discard till x goals are completed.\nx. play x loot cards.\nx. kill x monsters.\nx. give at least x¢ to another player at one time.\nx. purchase x items.\nx. roll a x three times. when x goals are completed, each player gains x treasure":
            return noTargetEffect(room.socialGoalsEffect(game, nr.numbers));
        case "each player discards their hands and loots x":
            return noTargetEffect(room.discardHandsAndLootEffect(game, nr.nextNumber()));
